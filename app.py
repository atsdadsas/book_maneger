...
# --- ★ここを追加！★ ---
    # 3. 週間スケジュール表を作るための「全データ」も取得する
    all_res = Reservation.query.all()
    # 検索しやすい辞書形式に変換（history関数でやっているのと同じこと）
...
def history():
    ...
    # --- 3. 全体の予約リスト（これは元々あったやつ） ---
    all_res = Reservation.query.all()
    res_dict = {(r.day, r.slot): r.user_name for r in all_res}

    # --- 4. HTMLに全部渡す（day, slot, res を追加！） ---
    return render_template('history.html', 
                           reservations=res_dict,
                           day=current_day,       # 追加
                           slot=current_slot,     # 追加
                           res=active_reservation # 追加
                           )
